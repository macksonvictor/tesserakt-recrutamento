...
import CatMovementData from "./Cat_Movement.json";
import RainbowCatData from "./black_rainbow_cat.json";
...
const ROLES = [
  {
    id: "cto", title: "CTO / Braço Direito", icon: "zap", color: "#f59e0b",
  ...
  {
    id: "ai", title: "Dev IA / ML", icon: "brain", color: "#8b5cf6",
  ...
  {
    id: "fullstack", title: "Dev Full Stack", icon: "monitor", color: "#10b981",
  ...
  {
    id: "robotics", title: "Dev Robótica", icon: "cpu", color: "#3b82f6",
  ...
  {
    id: "designer", title: "Designer UI/UX", icon: "palette", color: "#ec4899",
  ...
  {
    id: "growth", title: "Growth / Marketing", icon: "trendUp", color: "#f97316",
...
const SYSTEM = `Você é o Vesper — o conselheiro pessoal e mentor de um jovem fundador de 19 anos chamado Mackson, que está montando a Tesserakt: uma startup de tecnologia com produtos de IA, mobile e robótica.
...
Seu papel é ser o cara no ouvido do Mackson — como o Vesper no Kingsman. Você é extremamente experiente, direto, inteligente e fala como um amigo sênior que já viu tudo. Você não analisa de forma fria — você GUIA em tempo real.
...
async function triggerPrintExport(session, scoreData) {
  try {
    const res = await fetch("http://localhost:8000/gerar-pdf", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ session, score: scoreData })
    });
    ...
    a.href = url;
    a.download = `tesserakt-${session.candidateName.toLowerCase().replace(/\s+/g, "-")}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  } catch (e) {
    alert("Erro ao gerar PDF. Confirma que o backend está rodando:\ncd api && uvicorn main:app --reload");
  }
}
...
// ── SVG Icons ──────────────────────────────────────────────────
function Icon({ d, size=16, color="currentColor", strokeWidth=1.8, fill="none" }) {
  return (
...
// paths — Lucide Icons official paths
const I = {
  // Role icons
  ...
  cpu:       ["M12 17v5","M17 22v-5","M7 22v-5","M22 12h-5","M22 7h-5","M22 17h-5","M2 12h5","M2 7h5","M2 17h5","M7 2v5","M12 2v5","M17 2v5","M7 7h10v10H7z"],
  palette:   ["M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7h-1","M14.5 9a2.5 2.5 0 1 0-5 0 2.5 2.5 0 0 0 5 0z","M22 13h-4","M20 15v-4"],
  trendUp:   ["M22 7l-8.5 8.5-5-5L2 17","M16 7h6v6"],
  // UI icons
  history:   ["M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8","M3 3v5h5","M12 7v5l4 2"],
  chartBar:  ["M3 3v18h18","M18 17V9","M13 17V5","M8 17v-3"],
  search:    ["M21 21l-4.35-4.35","M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0"],
  // Quick action icons — corrected
  theater:   ["M2 12a10 10 0 1 0 20 0 10 10 0 0 0-20 0","M8 14s1.5 2 4 2 4-2 4-2","M9 9h.01","M15 9h.01"],
  flame:     ["M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"],
  pause:     ["M10 4H6v16h4V4z","M18 4h-4v16h4V4z"],
  skipFwd:   ["M5 4l10 8-10 8V4z","M19 4v16"],
  doorOut:   ["M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4","M16 17l5-5-5-5","M21 12H9"],
  ...
  questions: ["M8 6h13","M8 12h13","M8 18h13","M3 6h.01","M3 12h.01","M3 18h.01"],
  send:      ["M22 2L11 13","M22 2l-7 20-4-9-9-4 20-7"],
  check:     "M20 6L9 17l-5-5",
  arrowL:    "M19 12H5M12 5l-7 7 7 7",
  arrowR:    "M5 12h14M12 5l7 7-7 7",
  panelL:    ["M3 3h18v18H3z","M9 3v18"],
  panelR:    ["M3 3h18v18H3z","M15 3v18"],
};

function RoleIcon({ icon, size=18, color="currentColor" }) {
  const d = I[icon];
  if (!d) return null;
...
function LoadingCat() {
  ...
  useEffect(() => {
    const anim = lottie.loadAnimation({
      ...
      loop: true,
      autoplay: true,
      animationData: RainbowCatData,
    });
    return () => anim.destroy();
  }, []);
  return <div ref={ref} style={{width:56,height:56,flexShrink:0}}/>;
}

// Avatar do Vesper — parado, anima ao clicar
function VesperAvatar({ size=66, dark=true }) {
  const ref = useRef(null);
  ...
  return (
    <div
      ref={ref}
      onClick={() => { animRef.current?.stop(); animRef.current?.goToAndPlay(0, true); }}
      title="Clica no Vesper!"
      style={{
...
const QUICK_ACTIONS = [
  { label: "?",        desc: "O que perguntar agora?",     msg: "?",        icon: null },
  { label: "Perfil",   desc: "Leitura do perfil até agora",msg: "perfil",   icon: "user" },
  { label: "Blef?",    desc: "Está exagerando?",           msg: "blef?",    icon: "theater" },
  { label: "Convencer",desc: "Como apresentar a Tesserakt",msg: "convencer",icon: "flame" },
  { label: "Pausa",    desc: "Como usar o silêncio",       msg: "pausa",    icon: "pause" },
  { label: "Próxima",  desc: "Melhor próxima pergunta",    msg: "próxima",  icon: "skipFwd" },
  { label: "Desistir?",desc: "Vale continuar?",            msg: "desistir?",icon: "doorOut" },
];
  ...
  return (
    <div style={{maxWidth:760,margin:"0 auto",padding:"48px 16px 32px"}}>
      <div style={{textAlign:"center",marginBottom:44}}>
        ...
        <p style={{fontSize:12,color:theme.textMuted,marginBottom:22}}>O Vesper te guia em tempo real durante cada conversa</p>
        {historyCount > 0 && (
          <button onClick={onHistory}
            ...
            <Icon d={I.history} size={14} color="currentColor"/>Histórico
        ...
        {ROLES.map((role,i)=>(
          <button key={role.id} onClick={()=>onSelect(role)}
            ...
            <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:12}}>
              <div style={{...n(theme,true),width:40,height:40,borderRadius:11,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <RoleIcon icon={role.icon} size={18} color={role.color}/>
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:11,color:theme.textMuted}}>12 perguntas · Vesper ao vivo</span>
            </div>
          </button>
        ))}
      </div>
    ...
    <div style={{maxWidth:500,margin:"56px auto 0",padding:"0 16px"}}>
      <div style={{...n(theme),padding:34,borderRadius:26,animation:"fadeUp .4s ease"}}>
        <div style={{display:"flex",alignItems:"center",gap:13,marginBottom:28}}>
          <div style={{...n(theme,true),width:46,height:46,borderRadius:13,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <RoleIcon icon={role.icon} size={22} color={role.color}/>
          </div>
          <div>
        ...
        <button disabled={!name.trim()} onClick={()=>onStart({name:name.trim(),linkedin,obs})}
          style={{width:"100%",padding:"13px 0",borderRadius:13,border:"none",background:name.trim()?theme.text:theme.border,color:name.trim()?theme.bg:theme.textMuted,fontFamily:"inherit",fontSize:14,fontWeight:700,cursor:name.trim()?"pointer":"not-allowed",transition:"all .2s",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          Começar<Icon d={I.arrowR} size={14} color={name.trim()?theme.bg:theme.textMuted}/>
        </button>
        <button onClick={onBack}
          style={{width:"100%",marginTop:10,...n(theme,false,true),border:"none",padding:"10px 0",fontFamily:"inherit",fontSize:13,color:theme.textSub,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <Icon d={I.arrowL} size={13} color={theme.textSub}/>Voltar
        </button>
      </div>
    </div>
  );
...
function SummaryScreen({ session, onBack, t: theme }) {
  ...
  useEffect(()=>{ generateScore(session).then(s=>{setScore(s);setLoading(false);}); },[]);
    ...
    <div style={{maxWidth:660,margin:"0 auto",padding:"32px 16px 48px"}}>
      <div style={{...n(theme),padding:30,borderRadius:26,animation:"fadeUp .4s ease"}}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24}}>
          <div style={{...n(theme,true),width:52,height:52,borderRadius:15,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <RoleIcon icon={role?.icon} size={24} color={role?.color}/>
          </div>
          <div>
            ...
            {session.linkedin&&<div style={{fontSize:11,color:theme.textMuted,marginTop:2,display:"flex",alignItems:"center",gap:4}}><Icon d={I.link} size={11} color={theme.textMuted}/>{session.linkedin}</div>}
          </div>
        </div>
        ...
        {loading?(
          <div style={{textAlign:"center",padding:"48px 0"}}>
            <div style={{display:"flex",justifyContent:"center"}}><LoadingCat/></div>
            <div style={{fontSize:13,color:theme.textSub,marginTop:14}}>Vesper está avaliando...</div>
          </div>
        ):score?(
          <>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:12}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                ...
                <div style={{fontSize:12,color:theme.textSub,display:"flex",alignItems:"center",gap:5}}>
                  <Icon d={score.tem_fogo?I.flame:I.moon} size={13} color={score.tem_fogo?"#f97316":"#94a3b8"}/>
                  {score.tem_fogo?"Tem fogo":"Sem fogo"} · Fit: <strong>{score.fit_cultural}</strong>
                </div>
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              {[{title:"Pontos Fortes",items:score.pontos_fortes,color:"#16a34a",sign:"+"},{title:"Pontos de Atenção",items:score.pontos_atencao,color:"#d97706",sign:"⚠"}].map(({title,items,color,sign})=>(
            ...
            <div style={{background:theme.text,color:theme.bg,padding:18,borderRadius:14,fontSize:14,fontWeight:700,lineHeight:1.6,fontStyle:"italic",marginBottom:20}}>"{score.veredicto}"</div>
          </>
        ):(
          <div style={{color:theme.textSub,fontSize:14,textAlign:"center",padding:28}}>Não foi possível gerar avaliação.</div>
        )}
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <button onClick={()=>{setExporting(true);triggerPrintExport(session,score);setTimeout(()=>setExporting(false),2000);}}
            ...
            {exporting?"Gerando PDF...":<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Icon d={I.printer} size={14} color={theme.bg}/>Exportar PDF</span>}
          </button>
          <button onClick={onBack}
            style={{flex:1,minWidth:140,padding:"13px 0",borderRadius:13,border:"none",...n(theme,false,true),color:theme.textSub,fontFamily:"inherit",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
            <Icon d={I.arrowL} size={13} color={theme.textSub}/>Voltar
          </button>
        </div>
      </div>
    ...
    <div style={{maxWidth:660,margin:"0 auto",padding:"40px 16px 48px"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          {history.length>0&&(
            <button onClick={()=>{if(confirm("Apagar todo o histórico?")){saveHistory([]);setHistory([]);onDeleteAll();}}}
              style={{...n(theme,false,true),border:"none",padding:"8px 14px",fontFamily:"inherit",fontSize:12,color:"#e55",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
              <Icon d={I.trash} size={13} color="#e55"/>Apagar tudo
            </button>
          )}
          <button onClick={onBack} style={{...n(theme,false,true),border:"none",padding:"8px 14px",fontFamily:"inherit",fontSize:13,color:theme.textSub,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
            <Icon d={I.arrowL} size={13} color={theme.textSub}/>Voltar
          </button>
        </div>
      </div>
      {history.length>0&&(
        <div style={{...n(theme,true),padding:"10px 14px",borderRadius:12,marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
          <span style={{color:theme.textMuted,display:"flex",alignItems:"center"}}><Icon d={I.search} size={14} color={theme.textMuted}/></span>
              ...
              <div key={h.id} onClick={()=>onOpen(h)}
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:11,minWidth:0}}>
                    <div style={{...n(theme,true),width:40,height:40,borderRadius:11,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      <RoleIcon icon={role?.icon} size={18} color={role?.color}/>
                    </div>
...
function InterviewScreen({ session, onBack, onSummary, onSessionUpdate, t: theme, dark }) {
  ...
  // ── JSX inline — sem funções internas para evitar re-mount ──
  const sidebarJSX = (
    <div style={{padding:isMobile?"12px 12px 80px":"14px 12px 14px 14px",overflowY:"auto",height:"100%"}}>
      <div style={{...n(theme,true),padding:12,marginBottom:12,borderRadius:13}}>
        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:10}}>
          <div style={{...n(theme),width:32,height:32,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <RoleIcon icon={role.icon} size={15} color={role.color}/>
          </div>
          <div>
          ...
          {activeQ===i&&!answeredQ[i]&&(
            <div style={{marginTop:11}} onClick={e=>e.stopPropagation()}>
              ...
              <button disabled={!answerInput.trim()||loading} onClick={()=>submitAnswer(i,answerInput)}
                style={{border:"none",borderRadius:9,padding:"8px 0",width:"100%",fontFamily:"inherit",fontSize:11,fontWeight:700,cursor:answerInput.trim()&&!loading?"pointer":"not-allowed",marginTop:6,background:answerInput.trim()?theme.text:theme.border,color:answerInput.trim()?theme.bg:theme.textMuted,transition:"all .2s",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                Analisar com Vesper<Icon d={I.arrowR} size={12} color={answerInput.trim()?theme.bg:theme.textMuted}/>
              </button>
            </div>
          )}
          {answeredQ[i]&&(
      ...
      {answeredCount>=3&&(
        <button onClick={onSummary}
          style={{width:"100%",marginTop:8,padding:"11px 0",borderRadius:11,border:"none",background:theme.text,color:theme.bg,fontFamily:"inherit",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <Icon d={I.chartBar} size={13} color={theme.bg}/>Avaliação final
        </button>
      )}
    </div>
  );
  ...
  const chatJSX = (
    <>
      <div style={{padding:"10px 14px 4px",borderBottom:`1px solid ${theme.border}`,overflowX:"auto",display:"flex",gap:6,flexWrap:"nowrap",background:theme.bg}}>
        {QUICK_ACTIONS.map(({label,desc,msg,icon})=>(
          <button key={msg} onClick={()=>sendChat(msg)} disabled={loading} title={desc}
            style={{...n(theme,false,true),border:"none",padding:"5px 11px",fontFamily:"inherit",fontSize:11,color:theme.textSub,cursor:loading?"not-allowed":"pointer",whiteSpace:"nowrap",flexShrink:0,opacity:loading?0.4:1,transition:"all .15s",display:"flex",alignItems:"center",gap:5}}
            onMouseEnter={e=>{if(!loading)e.currentTarget.style.color=theme.text;}}
            onMouseLeave={e=>e.currentTarget.style.color=theme.textSub}>
            {icon&&<Icon d={I[icon]} size={12} color="currentColor"/>}
            {label}
          </button>
        ))}
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"14px 14px 8px"}}>
        {messages.map((msg,i)=>{
          const isLastAssistant = msg.role==="assistant" && !messages.slice(i+1).some(m=>m.role==="assistant") && !loading;
          return (
            <div key={i} style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",marginBottom:12,animation:"fadeUp .3s ease",alignItems:"flex-start"}}>
              {msg.role==="assistant"&&(
                <div style={{width:66,flexShrink:0,marginRight:10,marginTop:-30}}>
                  {isLastAssistant&&<VesperAvatar size={66} dark={dark}/>}
                </div>
              )}
        ...
        {loading&&(
          <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
            <LoadingCat/>
            <div style={{fontSize:12,color:theme.textSub,fontStyle:"italic"}}>Vesper está pensando...</div>
          </div>
        )}
        <div ref={bottomRef}/>
      </div>
  ...
  const notesJSX = (
    <div style={{flex:1,padding:14,display:"flex",flexDirection:"column"}}>
      <div style={{fontSize:13,fontWeight:700,color:theme.text,marginBottom:10,display:"flex",alignItems:"center",gap:6}}><Icon d={I.notes} size={14} color={theme.text}/>Suas notas</div>
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 60px)"}}>
        <div style={{display:"flex",borderBottom:`1px solid ${theme.border}`,background:theme.bg}}>
          {[["questions","Perguntas","questions"],["chat","Vesper","user"],["notes","Notas","notes"]].map(([tab,label,ico])=>(
            <button key={tab} onClick={()=>setMobileTab(tab)}
              style={{flex:1,padding:"10px 0",border:"none",background:"none",fontFamily:"inherit",fontSize:12,fontWeight:mobileTab===tab?700:400,color:mobileTab===tab?theme.text:theme.textSub,borderBottom:mobileTab===tab?`2px solid ${theme.text}`:"2px solid transparent",cursor:"pointer",transition:"all .15s",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
              <Icon d={I[ico]} size={13} color="currentColor"/>{label}
            </button>
          ))}
        </div>
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          {mobileTab==="questions"&&<div style={{flex:1,overflowY:"auto"}}>{sidebarJSX}</div>}
          {mobileTab==="chat"&&<>{chatJSX}</>}
          {mobileTab==="notes"&&notesJSX}
        </div>
      </div>
    );
  }
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:theme.chatBg,minWidth:0}}>
        <div style={{padding:"7px 12px",borderBottom:`1px solid ${theme.border}`,display:"flex",alignItems:"center",gap:8,background:theme.bg}}>
          <button onClick={()=>setSideOpen(!sideOpen)}
            style={{...n(theme,false,true),border:"none",width:26,height:26,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <Icon d={sideOpen?I.panelL:I.panelR} size={13} color={theme.textSub}/>
          </button>
          ...
          <button onClick={()=>setNotesOpen(!notesOpen)}
            style={{...n(theme,notesOpen,true),border:"none",padding:"4px 10px",fontFamily:"inherit",fontSize:11,color:notesOpen?theme.text:theme.textSub,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
            <Icon d={I.notes} size={12} color="currentColor"/>{notesOpen?"Fechar":"Notas"}
          </button>
        </div>
          ...
          {notesOpen&&(
            <div style={{width:220,borderLeft:`1px solid ${theme.border}`,display:"flex",flexDirection:"column",background:theme.bg,flexShrink:0}}>
              <div style={{padding:"10px 13px",borderBottom:`1px solid ${theme.border}`}}>
                <div style={{fontSize:11,fontWeight:700,color:theme.text,display:"flex",alignItems:"center",gap:5}}><Icon d={I.notes} size={12} color={theme.text}/>Suas notas</div>
        ...
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          {screen==="interview"&&activeSession&&Object.keys(activeSession.answeredQ||{}).length>=3&&(
            <button onClick={()=>setScreen("summary")}
              style={{...n(theme,false,true),border:"none",padding:"5px 11px",fontFamily:"inherit",fontSize:11,fontWeight:700,color:theme.text,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
              <Icon d={I.chartBar} size={13} color={theme.text}/>Avaliar
            </button>
          )}
          <button onClick={toggleDark}
            style={{...n(theme,false,true),border:"none",width:30,height:30,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <Icon d={dark?I.sun:I.moon} size={15} color={theme.text}/>
          </button>
          {screen!=="roles"&&(
            <button onClick={()=>{setHistory(loadHistory());setScreen("roles");}}
              style={{...n(theme,false,true),border:"none",padding:"5px 11px",fontFamily:"inherit",fontSize:12,color:theme.textSub,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
              <Icon d={I.arrowL} size={13} color={theme.textSub}/>Início
            </button>
          )}
        </div>
      </div>
